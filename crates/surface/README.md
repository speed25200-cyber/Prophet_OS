...
Elle imprime l'adaptateur, la scène, la médiane, le p95 et le maximum par image, la part du
processeur avant l'attente du GPU (composition, tessellation, soumission) et la mémoire
résidente. `--repos N` mesure la consommation au repos : il rejoue pendant N secondes la
politique de redessin de la fenêtre et imprime les images rendues et le temps processeur, en
tout et par phase (pleine cadence, cadence ralentie après trente secondes sans geste, veille
du champ après deux minutes sur un rastériseur logiciel, ADR 0055). Sur un rastériseur logiciel elle mesure le processeur ; sur une carte
...
L'en-tête dit où l'agent navigue (titre et adresse rendus par le service) et l'ouvre dans le
navigateur de l'humain ; l'onglet Parcours relit dans le journal ce que l'agent a touché : chaque appel d'outil avec sa cible contrôlée et son issue, jamais le contenu. Voir les ADR
