...
//! pour la demande identique qui suit ; une décision de tâche vaut pour toute la tâche, sauf
//! pour une action irréversible, qui s'autorise une fois (ADR 0054).
...
/// Un outil engageant et externe : `essai.envoi`, irréversible comme un envoi hors de la
/// machine ; `essai.publier`, externe mais réversible, comme une publication qu'on retire.
#[derive(Debug)]
struct Envoi {
    nom: &'static str,
    irreversible: bool,
...
[capabilities.max]
"fs.read" = ["~/docs/**"]
"tool.call" = ["essai.envoi", "essai.publier", "approval.wait"]
"#,
    )
    .unwrap();
    ...
    let grants = vec![
        ...
        Grant::new(Res::Tool, Act::Call, "essai.publier"),
...
fn appel(registry: &Registry, context: &ToolContext) -> CallResult {
    appel_de(registry, context, "essai.envoi")
}

fn appel_de(registry: &Registry, context: &ToolContext, outil: &str) -> CallResult {
    registry.call(outil, &json!({}), context, OffsetDateTime::now_utc())
}
...
#[test]
fn une_decision_de_tache_vaut_pour_toute_la_tache_sauf_l_irreversible_et_un_refus_est_definitif() {
    let dir = tempfile::tempdir().unwrap();
    let (context, registry, broker) = monde(dir.path());
    let autoriser_pour_la_tache = |outil: &str| {
        let r = appel_de(&registry, &context, outil);
    ...
    // Une action réversible : toute la tâche, les appels suivants passent sans nouvelle demande.
    autoriser_pour_la_tache("essai.publier");
    for _ in 0..3 {
        let r = appel_de(&registry, &context, "essai.publier");
        assert!(!r.is_error, "{r:?}");
    }
    assert!(broker.lock().unwrap().approvals().pending().is_empty());

    // Une action irréversible (ADR 0054) : la même décision de tâche ne vaut qu'une fois ;
    // l'appel suivant repasse devant l'humain.
    autoriser_pour_la_tache("essai.envoi");
    let r = appel(&registry, &context);
    assert!(!r.is_error, "{r:?}");
    let r = appel(&registry, &context);
    assert!(r.is_error, "{r:?}");
