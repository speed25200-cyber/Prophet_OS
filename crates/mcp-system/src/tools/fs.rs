...
file_tool!(
    Search,
    "fs.search",
    "Cherche par nom ou contenu dans les descendants autorisés d'un répertoire, ou dans un seul fichier si `root` en nomme un. Un nom qu'aucun fichier ne porte est cherché ensuite dans le contenu, et `note` le dit. Par contenu, rend pour chaque fichier le nombre exact de lignes qui contiennent le texte (`matching_lines`) et les cinq premières (numéro et extrait), à lire ensuite avec fs.read offset si besoin. Sans liens ; 200 résultats, 256 Kio par fichier, 8 Mio de contenu au total. Une recherche incomplète est signalée.",
    "fs.read",
    Search,
    "root",
    json!({
