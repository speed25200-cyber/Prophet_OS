...
def main():
    ...
    report = {"command": command, "weights_sha256": digest, "runs": []}
    ...
    with (args.output / "server.log").open("w") as log:
        ...
        try:
            ...
            while True:
                ...
                time.sleep(.1)
            env = dict(os.environ, PROPHET_TEST_ENDPOINT=endpoint + "/v1", PROPHET_TEST_MODEL=args.model)
            ...
            for number in range(1, args.repetitions + 1):
                ...
                with log_path.open("w") as test_log:
                    result = subprocess.run(
                        ["cargo", "test", "-p", "agentd", "--test", "local_daemon",
                         case, "--", "--ignored", "--exact", "--nocapture"],
