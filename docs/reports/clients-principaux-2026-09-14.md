...
- **Les approbations de bout en bout** (ADR 0041, `9772ede`) : une action que capd refuse
  ...
  Avant, une telle mission mourait sans que l'humain ait rien vu. Le terminal tranche aussi
  (`prophet cap approvals`, `approve`, `deny`, `rules`, `d83f6d5`) ; la surface accorde aussi pour
  toute la mission (`64db706`) ; et la voix tranche : « Prophète, accorde » / « refuse »
  (`d7ee0c4`).
...
| Une décision qui arrive est dite une fois (mission, question, conséquence, motif du modèle, « dites accorde ou refuse ») ; la même ne se redit pas, une autre si, une revenue si ; lecture coupée : muette | `crates/surface/src/missions.rs` (`dire_la_decision`) |
