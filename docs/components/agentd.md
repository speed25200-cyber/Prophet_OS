...
| `task.halt` | Arrêt d'urgence, sans paramètre : chaque mission en main s'arrête comme par `task.cancel` (travailleur prié de s'arrêter, séance conclue, client officiel tué par le lanceur, plan annulé) ; rend `cancel_requested`, `cancelled`, `unattended` (non finies mais menées par rien) et `errors` ; rien n'est publié ni défait |
