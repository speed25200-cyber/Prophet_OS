...
le fige ; il retire aussi les transitions et le défilement animé (la molette et le clavier
déplacent la page d'un coup), que le décocher rend. La barre du système ne relève que des comptes reçus ; le rail porte l'anneau des
...
L'onglet « Plan & accès » d'une mission menée par un client officiel dit aussi, sous « Réseau du client », les hôtes vers lesquels il pourra sortir par egress (`client_hosts`), toute autre destination étant refusée. L'application approuvée passe par `task.apply` et `task.undo` (ADR 0023). L'onglet Parcours relit le journal (`ledger.query` sur la mission, toutes les deux secondes, à partir du dernier événement lu et par pages de quatre cents : une longue mission se rattrape sans relire son début, et sa frise ne se fige pas sur ses premiers gestes) et montre chaque appel d'outil avec sa cible contrôlée et son issue, jamais le contenu lu ou écrit. Il se lit en deux frises : les états que le service a tenus, puis ce que l'agent a touché, un nœud par geste sur un rail (le texte d'un geste s'enroule dans sa colonne, le nœud tient à sa première ligne) — plein à l'accent pour un appel réussi, losange rouge pour un refus de capd (rattaché à l'appel refusé, avec son motif en mots : « hors de la portée de la mission », « droits retirés »…), losange orangé pour un livrable rappelé par le service (ADR 0049), vert pour une publication, point à l'accent vif pour une **sortie réseau** qu'egress a relayée (hôte, méthode, octets dans chaque sens — ni chemin ni contenu) et losange rouge pour celle qu'il a refusée ou bloquée comme exfiltration. Des sorties successives vers le même hôte, avec la même issue, se réunissent sur une ligne qui les compte (« 3 × CONNECT », « 2 fois ») : un client officiel ouvre des dizaines de connexions par mission, et elles ne recouvrent pas ses gestes ; un geste de l'agent entre deux sorties coupe la suite, la chronologie tient. Des pastilles comptent en tête les appels, les refus, les rappels et les sorties réseau ; au-delà de 880 points de large, les deux frises se tiennent côte à côte. Au-delà de deux cents lignes, la frise montre les deux cents dernières et le dit. Pendant l'exécution, l'onglet Proposition montre en direct les cinq derniers gestes de l'agent, avec la même frise ; le « dernier geste » de l'en-tête prend les mêmes libellés (« livrable rappelé », « refusé par capd ») et passe les sorties réseau relayées sans histoire — un client officiel parle sans cesse à son éditeur, ce n'est pas un geste — mais montre un refus ou une erreur réseau. Voir l'[ADR 0018](../adr/0018-examen-des-versions.md).
