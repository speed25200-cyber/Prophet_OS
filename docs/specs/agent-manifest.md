...
review  = ["driver:claude-code"]         # relecture d'un travail rendu, par un autre regard
...
4. `model.preferred` : au moins un élément ; chaque élément est `local:<nom>`, `driver:<pilote>` ou `api:<fournisseur>:<modèle>`. `model.roles`, facultatif, n'admet que les clés `reflect`, `execute`, `code` et `review`, chacune avec au moins une référence valide ; un catalogue de missions (agentd) exige de plus que chaque référence de rôle figure dans `preferred`, le rôle ne pouvant qu'y choisir.
