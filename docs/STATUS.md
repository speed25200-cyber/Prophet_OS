...
- [x] M8-T4 — Pilote `claude-code` (2026-09-12, 24b8338) — ligne de commande, environnement, détection de session ; le 23 septembre, le client lancé en mission tourne en cage (ADR 0056 : fichiers, services et session confinés ; réseau par egress seulement, sous un jeton borné aux hôtes de son éditeur)
...
- Clients officiels en cage (M8-T4 à M8-T6, ADR 0056, phase 1) : `prophet-pilotd` lançait
  ...
  noms, et sont exigés par le coureur d'isolation (vert sur `bbe9f63`). Phase 2, le même jour :
  la cage a son propre espace réseau ; le réseau du client ne sort que par egress, sous un jeton
  de capd borné aux hôtes de son éditeur (`PROPHET_CLIENT_HOSTS` les complète), que le lanceur
  pose sur chaque requête. Éprouvé avec le vrai egress et le vrai capd : le client en cage lit une
  page témoin par son proxy, la même adresse en direct ne mène nulle part, et un jeton forgé par
  le client ne passe pas. Reste à confirmer, avec un client connecté, que la liste des hôtes de
  chaque éditeur suffit.
...
- Le journal ne perd plus les événements du service (ADR 0059, FRONTIER journalisation
  ...
  partis. Chacun porte maintenant une clé d'idempotence, attend dans l'état d'agentd et part
  dans l'ordre (après chaque commande, toutes les deux secondes) ; `ledger.append` accepte
  `idem` et n'écrit pas deux fois la même clé, index relu à l'ouverture. Essais : magasin
  (doublon, réouverture), daemon du journal, service (journal arrêté, agentd redémarré pendant
  la panne, journal relancé : l'événement arrive une fois).
- Révocation persistante (FRONTIER, permissions ; ADR 0013 la laissait à construire) : un
  ...
  après redémarrage), vert après. De même pour la file d'approbations : les demandes en
  attente, les décisions récentes et les règles « pour toute la mission » disparaissaient au
  redémarrage ; elles sont gardées (`approbations.json`, écrit d'un coup) et relues. Essai du
  daemon rouge avant, vert après.
- Un conflit de publication n'est plus une impasse (ADR 0058, FRONTIER durabilité) : une
  ...
  l'échange revient à sa place), ou tout annuler sans toucher ce qui est à lui — et
  `task.undo` avec `keep_changes` rétablit le reste d'une publication dont il a repris des
  fichiers. Journal SFS rétrocompatible (`kept` absent quand vide), capd retranché avant de
  poursuivre, `kept` au journal, dans l'inspection et la réponse ; panneau « CONFLIT DE
...
- `just check` : **999 réussis, 0 échec, 75 ignorés** (journal sans perte ni doublon ; révocation et approbations persistantes ; code à choisir depuis Système ; conflits de publication ; code d'approbation ; sorties réseau au journal et dans le parcours ; longues missions ; journal rapide ; outils bornés ; conclusion des clients hors du runtime), format, clippy, contrôles
...
outils natifs n'écrivent plus dans les documents sans examen. Phase 2 : son réseau ne sort que
